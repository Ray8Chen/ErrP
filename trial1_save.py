...
expName = 'trial1'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/raychen/Desktop/BCI project/psychopy/trial1.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    left1 = visual.ShapeStim(
        win=win, name='left1', vertices='arrow',
        size=(0.5, 0.5),
        ori=-90.0, pos=(-0.4, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='blue',
        opacity=None, depth=0.0, interpolate=True)
    right1 = visual.ShapeStim(
        win=win, name='right1', vertices='arrow',
        size=(0.6, 0.6),
        ori=90.0, pos=(0.4, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='red',
        opacity=None, depth=-1.0, interpolate=True)
    ...
    # keep track of which components have finished
    L_LComponents = [No1left, No1right, LeftText]
    ...
    while continueRoutine and routineTimer.getTime() < 8.0:
        ...
        # if LeftText is starting this frame...
        if LeftText.status == NOT_STARTED and tThisFlip >= 5.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # update status
            LeftText.status = STARTED
            LeftText.setAutoDraw(True)
        
        # if LeftText is active this frame...
        if LeftText.status == STARTED:
            # update params
            pass
        
        # if LeftText is stopping this frame...
        if LeftText.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > LeftText.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                LeftText.tStop = t  # not accounting for scr refresh
                LeftText.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'LeftText.stopped')
                # update status
                LeftText.status = FINISHED
    ...
    # keep track of which components have finished
    R_RComponents = [No1left_2, No1right_2, RightText]
    ...
    while continueRoutine and routineTimer.getTime() < 8.0:
        ...
        # if RightText is starting this frame...
        if RightText.status == NOT_STARTED and tThisFlip >= 5.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'RightText.started')
            # update status
            RightText.status = STARTED
            RightText.setAutoDraw(True)
        
        # if RightText is active this frame...
        if RightText.status == STARTED:
            # update params
            pass
        
        # if RightText is stopping this frame...
        if RightText.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > RightText.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                RightText.tStop = t  # not accounting for scr refresh
                RightText.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'RightText.stopped')
                # update status
                RightText.status = FINISHED
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        # if left1 is starting this frame...
        if left1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            left1.frameNStart = frameN  # exact frame index
            left1.tStart = t  # local t and not account for scr refresh
            left1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(left1, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'left1.started')
            # update status
            left1.status = STARTED
            left1.setAutoDraw(True)
        
        # if left1 is active this frame...
        if left1.status == STARTED:
            # update params
            pass
        
        # if left1 is stopping this frame...
        if left1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > left1.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                left1.tStop = t  # not accounting for scr refresh
                left1.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'left1.stopped')
                # update status
                left1.status = FINISHED
                left1.setAutoDraw(False)
        
        # *right1* updates
        
        # if right1 is starting this frame...
        if right1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            right1.frameNStart = frameN  # exact frame index
            right1.tStart = t  # local t and not account for scr refresh
            right1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(right1, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'right1.started')
            # update status
            right1.status = STARTED
            right1.setAutoDraw(True)
        
        # if right1 is active this frame...
        if right1.status == STARTED:
            # update params
            pass
        
        # if right1 is stopping this frame...
        if right1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > right1.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                right1.tStop = t  # not accounting for scr refresh
                right1.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'right1.stopped')
                # update status
                right1.status = FINISHED
                right1.setAutoDraw(False)
